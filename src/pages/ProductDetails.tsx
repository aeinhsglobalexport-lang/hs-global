...
const ProductDetails = () => {
  ...
  const [resolvedRelatedImages, setResolvedRelatedImages] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    const loadImages = async () => {
      ...
      // Check if these are slab paths (start with /src/assets/Collection)
      const isSlabPaths = baseImages.some(img => 
        img.startsWith('/src/assets/Collection')
      );

      if (isSlabPaths) {
        // Resolve all slab image paths to URLs
        const urls = await Promise.all(
          baseImages.map(async (path) => {
            const url = await getImageUrl(path);
            return url || path; // fallback to path if resolution fails
          })
        );
        setResolvedImages(urls.filter(Boolean));
      } else {
        // Furniture or other images - use as-is
        setResolvedImages(baseImages);
      }
    };
  ...
  const product = useMemo(() => {
    ...
    // ⭐ PRICING LOGIC - USD based
    let displayPrice = "Price on Request";
    const isAvailable = resolved?.available !== false;

    if (!isAvailable) {
      displayPrice = "Currently Unavailable";
    } else if (resolved?.priceUSD) {
      // ⭐ Use priceUSD and format it
      displayPrice = formatPriceFromUSD(resolved.priceUSD);
    }
    ...
    return {
      ...
      category,
      subcategory,
      image: resolvedImages[0] || "/demo2.webp",
      images: resolvedImages.length > 0 ? resolvedImages : ["/demo2.webp"],
      price: displayPrice,
      moq,
      specs,
      description:
  ...
  // Resolve related product images for slabs
  useEffect(() => {
    const loadRelatedImages = async () => {
      if (product.category !== "slabs" || !product.relatedProducts.length) return;

      const imageMap: Record<string, string> = {};

      await Promise.all(
        product.relatedProducts.map(async (relatedProduct) => {
          const imagePath = relatedProduct.image;
          
          // Check if it's a slab path
          if (imagePath.startsWith('/src/assets/Collection')) {
            const url = await getImageUrl(imagePath);
            imageMap[relatedProduct.id] = url || imagePath;
          } else {
            imageMap[relatedProduct.id] = imagePath;
          }
        })
      );

      setResolvedRelatedImages(imageMap);
    };

    loadRelatedImages();
  }, [product.relatedProducts, product.category]);
  ...
  if (resolvedImages.length === 0 && resolved) {
    return (
  ...
  // Get the correct image for related product
  const getRelatedProductImage = (relatedProduct: { id: string; image: string }) => {
    if (product.category === "slabs") {
      return resolvedRelatedImages[relatedProduct.id] || relatedProduct.image;
    }
  ...
  return (
    <div className="pt-20 min-h-screen">
      ...
      <div className="container mx-auto px-4 pt-0">
        ...
        <div className="hidden md:flex items-center text-gray-600 text-sm bg-white/60 backdrop-blur rounded-lg px-3 py-2 w-fit">
        ...
        <div className="md:hidden flex items-center text-gray-600 text-sm bg-white/60 backdrop-blur rounded-lg px-3 py-2 w-fit">
        ...
        <div className="mt-6 flex items-center justify-center gap-3 flex-wrap">
          {product.images.slice(0, 5).map((img, idx) => (
            <button
              ...
              className={`relative w-24 h-16 rounded-lg overflow-hidden border transition transform hover:scale-[1.02] ${
                selectedImage === idx
            ...
            <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-4">
              <div
                className={`text-xl sm:text-2xl font-bold ${
                  product.available ? "text-gray-900" : "text-gray-600"
                }`}
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              {product.available ? (
                <>
                  {product.category === "slabs" ? (
                    ...
                    <button
                      ...
                      className="h-11 w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 rounded-full bg-black text-white border-2 border-black hover:bg-white hover:text-black shadow-lg hover:shadow-xl transition-all duration-300 font-semibold"
                    >
                    ...
                    // ⭐ Furniture in cart: Show quantity handler
                    <div className="w-full sm:w-auto">
                      <QuantityHandler product={product} />
                    </div>
                  ) : (
                    // ⭐ Furniture not in cart: Show add to cart
                    <AddToCartButton
                      product={product}
                      onPhoneVerificationRequired={() => openModal(product)}
                      className="h-11 w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 rounded-full bg-black text-white border-2 border-black hover:bg-white hover:text-black shadow-lg hover:shadow-xl transition-all duration-300"
                    />
                  )}
                  ...
                  {etsyUrl && (
                    <a
                      ...
                      className="h-11 w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 rounded-full bg-orange-500 text-white border-2 border-orange-500 hover:bg-white hover:text-orange-500 shadow-lg hover:shadow-xl transition-all duration-300 font-semibold"
                    >
                      Etsy
                    </a>
                  )}
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="h-11 w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 rounded-full bg-red-600 text-white border-2 border-red-600 hover:bg-white hover:text-red-600 shadow-lg hover:shadow-xl transition-all duration-300 font-semibold"
                >
