...
export const ProductsModernVariant: React.FC = () => {
  ...
  const galleryLoaders = useMemo(
    () =>
      import.meta.glob("/public/gallery/**/*.{webp,jpg,jpeg,png}", {
        as: "url",
      ...
      (async () => {
        try {
          ...
          const resolved = await Promise.all(
            slice.map((k) => galleryLoaders[k]())
          );
          
          if (mounted) {
            setGalleryPreview(resolved.map((u) => u.replace(/^\/public/, "")));
          }
        } catch {
          // Silent fail
        }
  ...
  const allOrderedIds = useMemo(
    () => [
      ...getOrderedSubcategoryIds("furniture"),
      ...getOrderedSubcategoryIds("slabs")
    ],
      ...
      if (!rafScheduled && latestSection) {
        ...
        requestAnimationFrame(() => {
          if (latestSection && !userInteractedRef.current) {
            ...
            const belongsToFurniture = getOrderedSubcategoryIds("furniture").includes(latestSection);
            if (belongsToFurniture && activeCategory !== "furniture") {
              setActiveCategory("furniture");
            } else {
              const slabsIds = getOrderedSubcategoryIds("slabs");
              if (slabsIds.includes(latestSection) && activeCategory !== "slabs") {
