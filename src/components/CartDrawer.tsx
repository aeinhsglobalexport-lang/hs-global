...
export const CartDrawer: React.FC = () => {
  const { formatPrice, currency, getCurrencySymbol, convertPrice } = useLocalization();
  ...
  // Calculate subtotal in INR, then convert
  ...
  const subtotal = useMemo(() => convertPrice(subtotalINR), [subtotalINR, convertPrice]);
  const tax = useMemo(() => subtotal * 0.18, [subtotal]); // 18% GST
  const shipping = useMemo(() => subtotal > convertPrice(5000) ? 0 : convertPrice(100), [subtotal, convertPrice]);
  const packaging = useMemo(() => convertPrice(50), [convertPrice]);
  ...
  const handlePay = async () => {
    ...
    const totalInINR = subtotalINR + (subtotalINR * 0.18) + (subtotalINR > 5000 ? 0 : 100) + 50;
                  ...
                  <div className="bg-white/60 backdrop-blur-md rounded-xl border border-black/10 p-4">
                    ...
                    <div className="mt-3 pt-3 border-t border-black/10 space-y-2 text-sm">
                      ...
                      <div className="flex items-center justify-between text-gray-700">
                        ...
                        <span>
                          {getCurrencySymbol()}{tax.toLocaleString('en-US', { 
                            minimumFractionDigits: 2, 
                            maximumFractionDigits: 2 
                          })}
                        </span>
                      ...
                      <div className="flex items-center justify-between text-gray-700">
                        <span>Shipping</span>
                        <span>
                          {shipping === 0 ? 'FREE' : `${getCurrencySymbol()}${shipping.toLocaleString('en-US')}`}
                        </span>
                      </div>
                      <div className="flex items-center justify-between text-gray-700">
                        <span>Packaging</span>
                        <span>
                          {getCurrencySymbol()}{packaging.toLocaleString('en-US')}
                        </span>
                      </div>
